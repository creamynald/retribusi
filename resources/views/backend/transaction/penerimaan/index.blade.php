            ...
            <div class="block-header block-header-default">
                ...
                @if (auth()->user()->upt_id == null)
                    <form action="" class="d-flex gap-2">
                        <select class="js-select2 form-select" id="val-select2" name="upt_id" data-placeholder="Pilih UPT.."
                            style="width: auto;">
                            <option></option>
                            @foreach ($upt as $item)
                                <option value="{{ $item->id }}"
                                    @if (isset($_GET['upt_id'])) @if ($_GET['upt_id'] == $item->id) selected @endif
                    ...
                    <tbody>
                        @forelse ($penerimaan as $index => $data)
                            <tr>
                                ...
                                <td class="d-none d-sm-table-cell">{{ $data->rincianObjek->kode }} - <span
                                        style="font-weight: 800;">{{ $data->rincianObjek->nama }}</span></td>
                                ...
                                <td class="text-center">
                                    ...
                                    @else
                                        <button type="button" class="btn btn-info" data-bs-toggle="modal"
                                            data-bs-target="#modal-slideleft{{ $data->id }}"><i
                            ...
                            @if (auth()->user()->upt_id == null)
                                <!-- Slide Left Modal -->
                                <div class="modal modal-lg fade" id="modal-slideleft{{ $data->id }}" tabindex="-1"
                                    role="dialog" aria-labelledby="modal-slideleft" aria-hidden="true">
                                            ...
                                            <div class="block block-rounded shadow-none mb-0">
                                                ...
                                                <div class="block-content fs-lg">
                                                    <div class="row block-title">
                                                        <div class="row d-flex">
                                                            <div class="col-6" style="font-weight: 900;">Jumlah Setoran
                                                            </div>
                                                            <div class="col-6" style="font-weight: 900;">:
                                                                @rp($data->jumlah)</div>
                                                        </div>
                                                        <div class="row d-flex">
                                                            <div class="col-6">Kode Rekening</div>
                                                            <div class="col-6">: {{ $data->rincianObjek->kode }}</div>
                                                        </div>
                                                        <div class="row d-flex">
                                                            <div class="col-6">Nama Rekening</div>
                                                            <div class="col-6">: {{ $data->rincianObjek->nama }}</div>
                                                        </div>
                                                        <div class="row d-flex">
                                                            <div class="col-6">Tanggal Penerimaan</div>
                                                            <div class="col-6">:
                                                                {{ \Carbon\carbon::parse($data->tgl_penerimaan)->format('d-m-Y') }}
                                                            </div>
                                                        </div>
                                                        <div class="row d-flex">
                                                            <div class="col-6">Tanggal Penyetoran</div>
                                                            <div class="col-6">:
                                                                {{ \Carbon\carbon::parse($data->tgl_penyetoran)->format('d-m-Y') }}
                                                            </div>
                                                        </div>
                                                        <div class="col-12 mt-2 mb-2">
                                                            @php
                                                                $fileExtension = pathinfo(
                                                                    url('storage/' . $data->bukti_pembayaran),
                                                                    PATHINFO_EXTENSION,
                                                                );
                                                                $fileUrl = url('storage/' . $data->bukti_pembayaran);
                                                            @endphp
                                                            @if (in_array(strtolower($fileExtension), ['jpg', 'jpeg', 'png']))
                                                                <img src="{{ $fileUrl }}" alt=""
                                                                    class="img-fluid mx-auto d-block">
                                                            @elseif (strtolower($fileExtension) === 'pdf')
                                                                <iframe src="{{ $fileUrl }}" class="col-12 mt-2 mb-2"
                                                ...
                                                <form action="{{ route('updateStatus') }}" method="post"
                                                    ...
                                                    <input type="hidden" name="status"
                                                        value="@if ($data->status != 1) {{ '1' }}@else{{ 'Belum Diverifikasi' }} @endif"
                                                        placeholder="Diverifikasi">
                                                    ...
                                                    <div
                                                        ...
                                                        <button type="submit"
                                                            class="btn @if ($data->status == 1) {{ 'btn-danger' }}@else{{ 'btn-success' }} @endif"
                                                            data-bs-dismiss="modal">
                                                            @if ($data->status == 1)
                                                                {{ 'Batalkan Verifikasi' }}@else{{ 'Verifikasi' }}
                                                            @endif
                                                        </button>
                                                    </div>
                                                </form>
